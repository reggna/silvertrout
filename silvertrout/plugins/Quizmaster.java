...
import jbt.commons.game.ScoreManager;


public class Quizmaster extends jbt.Plugin {
  ...
  public Quizmaster() {
    ...
    scoreManager = new ScoreManager(new File("/home/tigge/Personal"
        + "/Programming/jbt/plugins/Quizmaster_scores"));
  ...
  public void awardScore(String nick) {
    ...
    getNetwork().sendPrivmsg(channelName, "Rätt svar var \""
  ...
  public void newRound() {
    ...
    getNetwork().sendPrivmsg(channelName, "En ny omgång starts med 20 utvalda"
        + " frågor av " + questions.size() + " totalt.");
  ...
  public void endRound() {
    getNetwork().sendPrivmsg(channelName, "Omgången är slut. Skriv !start för att"
        + " starta en ny omgång.");
    running = false;
  }
  ...
  public void endQuestion(String winner) {
    ...
    if(winner == null) {
      getNetwork().sendPrivmsg(channelName, "Rätt svar var \""
          + currentQuestion.answer + "\". Ingen lyckades svara rätt.");
      unanswerdQuestions++;
    } else {
